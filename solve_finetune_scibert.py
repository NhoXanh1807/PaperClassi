#!/usr/bin/env python3
"""
Paper Classification — Fine-tuned SciBERT + Ordinal Threshold Optimization
...
combined.to_csv(DATA_DIR / "submission_scibert.csv",         index=False)
pub_sub.to_csv(DATA_DIR  / "public_submission_scibert.csv",  index=False)
priv_sub.to_csv(DATA_DIR / "private_submission_scibert.csv", index=False)

# Save OOF + test probs for later ensemble
np.savez(DATA_DIR / "scibert_probs.npz", oof=oof_probs, test=test_probs)

print(f"\n{'='*45}")
print(f"Final OOF QWK : {final_qwk:.4f}")
print(f"Submission    : submission_scibert.csv ({len(combined)} rows)")
print("Pred dist:")
